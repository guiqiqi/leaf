...
5. **原生并发支持**
通过 `Leaf` 实现的主从模式分布架构，可以轻松的部署多进程系统(例如使用 `Gunicorn`)，甚至将 `Leaf` 部署在一组服务器上；而在使用时，您无需担心多个进程运行造成的事件调度混乱、插件多次运行、数据提交重复等等问题 —— 我们已经替您考虑到了。
6. **插件系统**
